...
**Phase:** Phase 33-B.3.C "Scoring transparency" shipped (v0.60, 2026-05-04). Third bucket of the 6-bucket v0.57 playtest response. Three opacities closed: (1) judge-reveal modal blocks input until Continue is clicked (was auto-dismissing in 2.4s; players couldn't read the taste rule before clicking through), (2) per-card breakdown hover on each scoring card lists every contribution with attribution — base ×stars, augments, items, modifier mult, and the judge taste's per-card rule (e.g. "Quaintness: Ostentatious ×0.7", "Narrative: held 4 rounds ×1.32"), (3) the Held N rounds bonus is now legible as a Narrative-taste line per card. Sim greedy n=200 unchanged (45.5% vs v0.59 46% — within seed noise; scoring math untouched, only labels added).
...
**6-phase rollout:** 27 Judge Spine ✅ → 28 Aesthetic Tags ✅ → 29 Visible Rival ✅ → 30 Plinth Composition ✅ → 31-A Target curve recalibration ✅ → 31-B.1 Run Modifiers ✅ → 31-B.2 Exhibition Refit ✅ → 31-B.3 Tag-granting items ✅ → 32 Theme Pass ✅ → 33-A Sporal completion ✅ → 33-B.0 RunLog refresh + digest tool ✅ → 33-B.1 Tag legibility surface ✅ → 33-B.2.0 Sim overhaul ✅ → 33-B.2.1 Target curve recalibration v2 ✅ → 33-B.3.A Rival threat layer ✅ → 33-B.3.B Generous-modifier bite pass ✅ → **33-B.3.C Scoring transparency ✅**. Living Exhibition redesign feature-complete; v0.57 playtest response in progress (6 buckets, A+B+C done, D-F pending).

**Next action:** Phase 33-B.3.D — Tooltip + copy cleanup. Plan: `design_log/phase_33_B_3_plan.md` Bucket D. Five rough edges: (1) item/augment tooltips leak internal jargon (`axis`, `5-tag` etc.) and don't say *who* benefits; (2) pair-combo copy is prescriptive ("forms a pair with X") — should be stative ("+N when paired with X"); (3) build-archetype HUD chips (Emotional Spectrum etc.) have no tooltip explaining requirements/effect; (4) augment scope is unclear — players think they apply only at acquisition; (5) "Passives activate this round" Eccentricity hint needs clarifying or the taste itself reconsidering. Files: `src/items.js`, `src/cards.js`, `src/augments.js` (description strings), `web/app.js` `makeSynergyTooltip` + new `makeArchetypeTooltip`, pair-combo copy, `src/judges.js` Eccentricity hint. Acceptance: zero `axis` / `5-tag` / `auraMult` jargon in user-visible strings; every archetype chip has a tooltip; pair-combo strings are stative; augment tooltip explicitly states "Applies for the rest of the run". Open question for next conversation: does the Eccentricity taste survive (player couldn't identify which cards it rewards)? Buckets after D: (E) Plinth L6+ + stale tier-odds; (F) Sporal Taxonomy Badge + mid-run unlock timing. Held / deferred: Crystalline/Plasmic species buffs (need targeted playtest), 0.68 judge-scale removal, Puffzak T2 aura, 66 unit tests rewrite, `src/ranking.js` cleanup, Refit-pool weighting follow-ups, Patron Subsidy / Cheap Plinths sim 61%.
