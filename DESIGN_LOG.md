...
**Phase:** Phase 33-B.3.E "Plinth L6+ + stale tier-odds tooltip" shipped (v0.62, 2026-05-08). Fifth bucket of the 6-bucket v0.57 playtest response. Two rough edges closed: (1) at L6 the plinth-add button used to read "Upgrade Exhibit (0 ✦)" and remain enabled — the old gate `S.human.level >= 9` never fired because the actual cap is `MAX_BOARD = 6`, not `MAX_LEVEL = 9`. Click was a no-op (`addPlinth` rejects when `level >= MAX_BOARD`). New gate: `plinthCost() === 0` → "Exhibit Maxed", disabled. (2) Exhibit-info tooltip used to iterate `lvl = 3; lvl <= 9`, listing odds for L7-9 the player can never reach — this was the "stale" copy flagged in the playtest plan (no literal "based on rounds" text existed). Now caps at `min(MAX_LEVEL, MAX_BOARD) = 6`. Subhead tightened to "Adds one display slot · shifts shop tier odds". `MAX_BOARD` now exported from `src/game.js` so the cap is sourced once.
