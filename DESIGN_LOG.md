...
**Phase:** Phase 33-B.3.D "Tooltip + copy cleanup" shipped (v0.61, 2026-05-04). Fourth bucket of the 6-bucket v0.57 playtest response. Six rough edges closed: (1) all seven pair-combo card descriptions rewritten from imperative ("Pair with X (+N each)") to stative ("+N each when paired with X"); (2) Vexborg + Clattorb (Carapace Lattice combo) now expose the +50 pair bonus in their descriptions — was completely invisible despite being a real 50/50 combo; (3) every HUD build-archetype chip (Plasma Cascade, Void Assembly, Spore Engine, Crystal Lattice, Chitin Wall, Emotional Spectrum, Patient Collection, Star Collector) now has a hover tooltip stating its trigger requirement and clarifying that archetypes are flavor labels, not stand-alone bonuses; (4) augment-badge tooltip appends "Applies for the rest of the run." line, addressing the "augments are confusingly one-shot-feeling" complaint; (5) Eccentricity taste hint rewritten from "Cards whose passive activated this round score ×1.6" (opaque — players couldn't tell which) to "×1.6 to specimens whose adjacency, pair-combo, round-timing, or aura passive fires this round"; (6) Mastery Protocol description "(Axis 4, 6, 6+4)" jargon stripped — now "All multiplier passives gain +0.1 to their multiplier". No mechanical changes; pure copy + tooltip plumbing.
...
**6-phase rollout:** 27 Judge Spine ✅ → 28 Aesthetic Tags ✅ → 29 Visible Rival ✅ → 30 Plinth Composition ✅ → 31-A Target curve recalibration ✅ → 31-B.1 Run Modifiers ✅ → 31-B.2 Exhibition Refit ✅ → 31-B.3 Tag-granting items ✅ → 32 Theme Pass ✅ → 33-A Sporal completion ✅ → 33-B.0 RunLog refresh + digest tool ✅ → 33-B.1 Tag legibility surface ✅ → 33-B.2.0 Sim overhaul ✅ → 33-B.2.1 Target curve recalibration v2 ✅ → 33-B.3.A Rival threat layer ✅ → 33-B.3.B Generous-modifier bite pass ✅ → 33-B.3.C Scoring transparency ✅ → **33-B.3.D Tooltip + copy cleanup ✅**. Living Exhibition redesign feature-complete; v0.57 playtest response in progress (6 buckets, A+B+C+D done, E-F pending).

**Next action:** Phase 33-B.3.E — Plinth L6+ + stale tier-odds tooltip. Plan: `design_log/phase_33_B_3_plan.md` Bucket E. Two issues: (1) at L6, plinth-add button shows $0 cost but click does nothing — should hide or read "Maxed"; (2) tier-odds tooltip text references "based on rounds" which is stale (current system doesn't ramp tier odds by round). Files: `web/app.js` (plinth-add gating, `onAddPlinth`, `btn-plinth` tooltip), `web/index.html` (tier-odds tooltip text). Acceptance: at L6 the button is hidden or non-clickable with "Maxed" copy, never shows $0; tier-odds tooltip describes the current per-level weight system without "based on rounds" copy. After E: (F) Sporal Taxonomy Badge + mid-run unlock timing. Held / deferred (don't pull in this phase): Crystalline/Plasmic species buffs (need targeted playtest before any buffs), 0.68 judge-scale removal, Puffzak T2 aura, 66 unit tests rewrite, `src/ranking.js` cleanup, Refit-pool weighting follow-ups, Patron Subsidy / Cheap Plinths sim 61%, redundant chapter-end `updateAggression` cleanup, Eccentricity taste fate (hint clarified in D — see if v0.61 playtest still flags it).
