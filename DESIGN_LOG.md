...
**Phase:** Phase 33-B.2.0 "Sim economic-realism overhaul" shipped (sim-only, no version bump 2026-05-03). First human playtest read (9 runs at v0.56) revealed the sim was systematically under-modelling skilled human economy: 9/9 wins, 8/9 perfect 3-life. Sim's `chitinous-stack` was reading 48.5%, but the human strategy (park at 25g interest cap, sell aggressively, reroll deeply when banked) wasn't in the policies. After the rewrite, sim survival jumped: greedy 36→89.5%, chitinous 48.5→87.5%, sporal 19→96.5%, wide 37→95.5% — matching the player's "the game is too easy" reading. The previously-suspected Chitinous dominance turned out to be a sim artifact; in the new sim Chitinous is mid-pack, Crystalline (71.5%) and Plasmic (74%) are the actual weak species, and the curve-pacing problem dwarfs all species-level imbalances.
...
**6-phase rollout:** 27 Judge Spine ✅ → 28 Aesthetic Tags ✅ → 29 Visible Rival ✅ → 30 Plinth Composition ✅ → 31-A Target curve recalibration ✅ → 31-B.1 Run Modifiers ✅ → 31-B.2 Exhibition Refit ✅ → 31-B.3 Tag-granting items ✅ → 32 Theme Pass ✅ → 33-A Sporal completion ✅ → 33-B.0 RunLog refresh + digest tool ✅ → 33-B.1 Tag legibility surface ✅ → **33-B.2.0 Sim overhaul ✅**. Living Exhibition redesign feature-complete; pacing iteration ahead.

**Next action:** Phase 33-B.2.1 — recalibrate the R8–R24 target curve against the now-trustworthy sim. Player playtest sat at 150–300% of target across the late game; new sim baselines confirm the targets are 30–40% too low. Per-round profile via `scripts/profile_targets.js` (n=300, greedy + species-stack policies), then bump R12–R24 targets to land greedy median in 30–45% survival band (the design intent from Phase 31-A). After curve fix, then look at Crystalline/Plasmic species buffs (the genuine spread issue) and the rival-aggro escalation signal (chapter-record DDA never escalates because score margins are huge — needs lives-saved or per-round-margin trigger). Affordance bug list from playtest also queued: judge-reveal Continue button; "held N rounds" bonus invisibility; per-augment/item modifier visibility in scoring breakdown; plinth past L6 button bug; pair-combo prescriptive copy; build-bonus tooltips; Sporal Taxonomy Badge bug; mid-run unlock timing. Other deferred: non-tag-taste Refit Pool weights uniform (Phase 31-B.2); tag-grant item offer-pool weighting (Phase 31-B.3); 66 unit tests rewrite (Phase 27 chore); `src/ranking.js` cleanup; Puffzak's bypassed +15% Sporal aura (T2, lower priority).
