...
**Phase:** Phase 33-B.1 "Tag legibility surface" shipped (v0.56, 2026-05-02). Aesthetic tags (Phase 28) now have a durable, art-pass-proof visibility layer: per-tag color stripe on card bottom, full readout in the existing hover tooltip (native + item-granted, distinguished), and an aggregate tag-mix pill in `#hud-sub`. Tag colors align with the Phase 32 per-taste color bands so the player builds one mental color map across judges + cards + HUD.

**Direction:** Game is now framed as **curation under shifting judge tastes** (the original re-frame goal). Judges are the scoring spine (11 tastes, 20 judges); aesthetic tags drive 5 of the 11 tastes; a visible AI rival competes for cards on a shared 8-slot persistent shop; the plinth is a *composition* (cap 6, adjacency-aware, pair-combos, live recalc on drag); each run draws one of 12 random modifiers; between chapters the player faces a paid Refit modal; items can re-aim wrong-taste cards via aesthetic tag-grants. UI/copy is theme-aligned: Appraisal / Reputation / Lustres ✦ / Open Showing / "the salon dismisses you".

**6-phase rollout:** 27 Judge Spine ✅ → 28 Aesthetic Tags ✅ → 29 Visible Rival ✅ → 30 Plinth Composition ✅ → 31-A Target curve recalibration ✅ → 31-B.1 Run Modifiers ✅ → 31-B.2 Exhibition Refit ✅ → 31-B.3 Tag-granting items ✅ → 32 Theme Pass ✅ → 33-A Sporal completion ✅ → 33-B.0 RunLog refresh + digest tool ✅ → **33-B.1 Tag legibility surface ✅**. Living Exhibition redesign feature-complete.

**Next action:** Phase 33-B.2 — capture and analyze the first human playtest run with the v0.56 logger + tag-visibility surface. Hand the raw runlog through `node scripts/digest_runlog.js <path>` and read the digest, watching specifically for: (a) Chitinous-stack feel (sim 48.5% pre-33-A — still upper-edge, possibly runaway-dominant); (b) greedy mixed-mod 45% median sits on upper edge of 30–45% band — don't add new generosity without recalibrating; (c) Brutal Curation modifier bite under deliberate (vs greedy) play. Other deferred follow-ups still pending: non-tag-taste Refit Pool weights uniform (Phase 31-B.2); tag-grant item offer-pool weighting (Phase 31-B.3); 66 unit tests rewrite (Phase 27 chore); `src/ranking.js` cleanup (still wired despite CLAUDE.md saying it's gone); Puffzak's bypassed +15% Sporal aura (T2, lower priority — Sprangus alone moved Sporal-stack into band).
