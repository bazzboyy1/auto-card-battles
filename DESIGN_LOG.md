...
**Phase:** Phase 33-B.2.1 "Target curve recalibration v2" shipped (v0.57, 2026-05-03). Closed the loop on the sim/playtest gap: with the post-overhaul sim landing greedy at 89.5% (vs the design intent of 30–45%), the R1–R24 ROUND_TARGETS curve was scaled up. R1–3 ×1.24, R4–7 ×1.34, R8–24 ×1.40. Greedy now lands at 50.5% survival (200 seeds), with R8 86% pass (first checkpoint), R11–12 / R17–19 ~80% (mid kill-zones), R24 finale 65% (real boss test). Crystalline 20.5% / Plasmic 23.5% are now visible as the genuine bottom of the species spread.
...
**6-phase rollout:** 27 Judge Spine ✅ → 28 Aesthetic Tags ✅ → 29 Visible Rival ✅ → 30 Plinth Composition ✅ → 31-A Target curve recalibration ✅ → 31-B.1 Run Modifiers ✅ → 31-B.2 Exhibition Refit ✅ → 31-B.3 Tag-granting items ✅ → 32 Theme Pass ✅ → 33-A Sporal completion ✅ → 33-B.0 RunLog refresh + digest tool ✅ → 33-B.1 Tag legibility surface ✅ → 33-B.2.0 Sim overhaul ✅ → **33-B.2.1 Target curve recalibration v2 ✅**. Living Exhibition redesign feature-complete; pacing iteration ahead.

**Next action:** Phase 33-B.3 — capture a second human playtest run against the v0.57 curve, then triage in this order: (a) does skilled play still trivialise the curve (i.e., does the user still hit 8/9 perfect runs)? If yes, bump factor to 1.50 or add modifier bite; (b) Crystalline/Plasmic species buffs (sim 20.5/23.5% is the genuine bottom of the spread); (c) rival-aggro escalation signal (chapter-record DDA is invisible because banked-play margins are still huge — needs lives-saved or per-round-margin trigger); (d) affordance bug list from first playtest: judge-reveal Continue button; "held N rounds" bonus invisibility; per-augment/item modifier visibility in scoring breakdown; plinth past L6 button bug; pair-combo prescriptive copy; build-bonus tooltips; Sporal Taxonomy Badge bug; mid-run unlock timing. Other deferred: non-tag-taste Refit Pool weights uniform (Phase 31-B.2); tag-grant item offer-pool weighting (Phase 31-B.3); 66 unit tests rewrite (Phase 27 chore); `src/ranking.js` cleanup; Puffzak's bypassed +15% Sporal aura (T2, lower priority).
