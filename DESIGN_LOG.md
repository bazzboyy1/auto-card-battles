...
**Phase:** Phase 31-B.2 "Exhibition Refit" shipped (v0.51, 2026-05-02). 6-phase "Living Exhibition" redesign on track; Phase 31-B sub-steps B.1 ✅ + B.2 ✅; B.3 tag-granting items still pending. Interest cap (`MAX_INTEREST=5`) already in place pre-Phase 31-B.

**Direction:** Game is being re-framed from *score execution game* to **curation under shifting judge tastes.** Judges are the scoring spine; aesthetic tags drive 4 of the 10 tastes; a visible AI rival competes for cards on a shared 8-slot persistent shop; the plinth is now a *composition* (cap 6, adjacency-aware, pair-combos, live recalc on drag); each run draws one of 12 random modifiers that reshape its rules; between chapters the player faces a paid Refit modal to spend hoarded gold.

**6-phase rollout:** 27 Judge Spine ✅ → 28 Aesthetic Tags ✅ → 29 Visible Rival ✅ → 30 Plinth Composition ✅ → 31-A Target curve recalibration ✅ → 31-B.1 Run Modifiers ✅ → **31-B.2 Exhibition Refit ✅** → 31-B.3 Tag-granting items → 32 Theme Pass. Each phase/sub-step shippable independently.

**Next action:** Begin Phase 31-B.3 — Tag-granting items. Convert the existing item pool (or a subset of it) so items grant aesthetic tags to attached cards (e.g. a "Velvet Drape" item grants Restrained, "Gilded Frame" grants Ostentatious). Plumb tag-granting through `cardHasTag()` in `src/judges.js` so judge tag-tastes read item-granted tags too. Goal: items become *judge-aligned context-variable rewards* (per `reinforcement-feedback-systems.md`) instead of flat % buffs, so a "wrong-taste card" can be re-aimed via item attachment. Spec lives in `design_log/phase_27_plan.md` "The Economy Fix" section + cut-list ("Items become judge-aligned").
