...
**Phase:** Phase 31-B.1 "Run Modifiers" shipped (v0.50, 2026-05-01). 6-phase "Living Exhibition" redesign on track; Phase 31-B is split into 3 sub-steps (B.1 ✅, B.2 Refit pending, B.3 tag-granting items pending). Interest cap was already in place (`MAX_INTEREST=5`) before Phase 31-B started — the `phase_27_plan.md` deliverable is moot.

**Direction:** Game is being re-framed from *score execution game* to **curation under shifting judge tastes.** Judges are the scoring spine; aesthetic tags drive 4 of the 10 tastes; a visible AI rival competes for cards on a shared 8-slot persistent shop; the plinth is now a *composition* (cap 6, adjacency-aware, pair-combos, live recalc on drag); each run draws one of 12 random modifiers that reshape its rules.
...
**Next action:** Begin Phase 31-B.2 — Exhibition Refit gold sink between chapters. Modal at chapter-end (after R8/R16/R23 scoring, before next chapter starts) offering paid options: swap a card (cost X), re-tier a card (cost Y), peek at the next judge before committing (cost Z). Wire into `Run.runBattle()` chapter-end branch (already exists at the rival-aggression hook). Goal is to give hoarded gold a chapter-boundary purpose; pairs with the existing interest cap to break the coasting loop. Spec lives in `design_log/phase_27_plan.md` "The Economy Fix" section.
