...
**Phase:** Phase 33-A "Sporal completion pass" shipped (v0.54, 2026-05-02). 6-phase Living Exhibition redesign complete (27→32) and Sporal residual fix landed; next is Phase 33-B (first human playtest read).
...
**6-phase rollout:** 27 Judge Spine ✅ → 28 Aesthetic Tags ✅ → 29 Visible Rival ✅ → 30 Plinth Composition ✅ → 31-A Target curve recalibration ✅ → 31-B.1 Run Modifiers ✅ → 31-B.2 Exhibition Refit ✅ → 31-B.3 Tag-granting items ✅ → 32 Theme Pass ✅ → **33-A Sporal completion ✅**. Living Exhibition redesign feature-complete.

**Next action:** Phase 33-B — first human playtest of the full redesign. Living Exhibition is feature-complete; capture a real run via the existing RunLog (v0.42) and read where the curve actually feels off vs the sim numbers. Specifically watch (a) Chitinous-stack feel (sim 48.5%, was 50.5% pre-33-A — still upper-edge, possibly runaway-dominant); (b) greedy mixed-mod 45% median sits on upper edge of 30–45% band — don't add new generosity without recalibrating; (c) Brutal Curation modifier bite under deliberate (vs greedy) play. Other deferred follow-ups still pending: non-tag-taste Refit Pool weights uniform (Phase 31-B.2); tag-grant item offer-pool weighting (Phase 31-B.3); 66 unit tests rewrite (Phase 27 chore); `src/ranking.js` cleanup (still wired despite CLAUDE.md saying it's gone); Puffzak's bypassed +15% Sporal aura (T2, lower priority — Sprangus alone moved Sporal-stack into band).
