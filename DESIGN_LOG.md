...
**Phase:** Phase 33-B.3.B "Generous-modifier bite pass" shipped (v0.59, 2026-05-04). Second bucket of the 6-bucket v0.57 playtest response. Each of the four generous modifiers now carries a paired downside the player can play around. Bull Market: free rerolls + interest cap shrunk 5→3 (banking ceiling 25g→15g). Generous Patron: +2/round + bench tax 2 ✦/round per bench specimen above 5. Curator's Stipend: +6 ✦/chapter + Refit-action premium +3 ✦. Curator's Pet: favored mult 1.4→1.25 (scorned 0.7 unchanged). Sim greedy deltas vs v0.58: Bull Market 57→62 (sim adapts to lower cap by spending more — bite is human-feel only), Generous Patron 77.5→53 (-24.5pp ✓), Stipend 51→51 (sim doesn't run refit phase — human-only), Curator's Pet 48.5→45 (-3.5pp baseline shift).
...
**6-phase rollout:** 27 Judge Spine ✅ → 28 Aesthetic Tags ✅ → 29 Visible Rival ✅ → 30 Plinth Composition ✅ → 31-A Target curve recalibration ✅ → 31-B.1 Run Modifiers ✅ → 31-B.2 Exhibition Refit ✅ → 31-B.3 Tag-granting items ✅ → 32 Theme Pass ✅ → 33-A Sporal completion ✅ → 33-B.0 RunLog refresh + digest tool ✅ → 33-B.1 Tag legibility surface ✅ → 33-B.2.0 Sim overhaul ✅ → 33-B.2.1 Target curve recalibration v2 ✅ → 33-B.3.A Rival threat layer ✅ → **33-B.3.B Generous-modifier bite pass ✅**. Living Exhibition redesign feature-complete; v0.57 playtest response in progress (6 buckets, A+B done, C-F pending).

**Next action:** Phase 33-B.3.C — Scoring transparency. Plan: `design_log/phase_33_B_3_plan.md` Bucket C. Playtest notes flagged 4–5 related opacities. (1) Judge-reveal modal can be clicked through — needs Continue button + input blocker. (2) "Held N rounds" bonus (8% per round per card) is invisible — no breakdown line. (3) Augment/item contributions show as bare `+SCORE` floats with no name or attribution. (4) Per-card hover should show that card's full breakdown. Approach: extend `calcBaseBreakdown` to return labeled lines for held-rounds + augments + items; judge-mult lines named in `src/judges.js`; web/app.js renders them in the breakdown modal and in per-card hover. Files: `src/game.js` calcBaseBreakdown (verify augment/item/held-rounds line items), `src/judges.js` named judge mults, `web/app.js` modal renderer + per-card hover + judge-reveal Continue, `web/index.html` + `web/style.css` for the Continue button. Acceptance: scoring breakdown for a 6-card board with 1 augment + 2 attached items has every line attributable (named + base/mult tag + value); judge-reveal modal blocks input until Continue. Buckets after C: (D) Tooltip + copy cleanup; (E) Plinth L6+ + stale tier-odds; (F) Sporal Taxonomy Badge + mid-run unlock timing. Held / deferred: Crystalline/Plasmic species buffs (need targeted playtest), 0.68 judge-scale removal, Puffzak T2 aura, 66 unit tests rewrite, `src/ranking.js` cleanup, Refit-pool weighting follow-ups, Patron Subsidy / Cheap Plinths sim 61% (not in this bucket's scope; flag for next playtest read).
