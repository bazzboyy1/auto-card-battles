...
**Phase:** Phase 33-B.3.A "Rival threat layer" shipped (v0.58, 2026-05-04). First bucket of the 6-bucket v0.57 playtest response. Reversed Phase 29's ≤10% Buster-principle aggro cap — playtest read it as no-pressure precisely because the bias was sub-perception. New aggro range [-0.20, +0.50] with per-round bumps (not just chapter-end), HUD threat pill (Distracted / Watching / Hunting / Pouncing), Mimic now contests last-bought species *this round* (was stale dominant-species majority), Specialist first-pick biases toward player's dominant species when aggro ≥ Hunting. Greedy 50.5% → 46.0% (-4.5pp, in band). Chitinous-stack 40% → 30.5% (-9.5pp — bite landed on the playtest target). Aggro ramp R1 Watching → R3 Hunting → R7 Pouncing on a strong run.
...
**6-phase rollout:** 27 Judge Spine ✅ → 28 Aesthetic Tags ✅ → 29 Visible Rival ✅ → 30 Plinth Composition ✅ → 31-A Target curve recalibration ✅ → 31-B.1 Run Modifiers ✅ → 31-B.2 Exhibition Refit ✅ → 31-B.3 Tag-granting items ✅ → 32 Theme Pass ✅ → 33-A Sporal completion ✅ → 33-B.0 RunLog refresh + digest tool ✅ → 33-B.1 Tag legibility surface ✅ → 33-B.2.0 Sim overhaul ✅ → 33-B.2.1 Target curve recalibration v2 ✅ → **33-B.3.A Rival threat layer ✅**. Living Exhibition redesign feature-complete; v0.57 playtest response in progress (6 buckets, A done, B-F pending).

**Next action:** Phase 33-B.3.B — Generous-modifier bite pass. Plan: `design_log/phase_33_B_3_plan.md` Bucket B. Sim sweep + playtest both flagged Bull Market / Generous Patron / Curator's Stipend / Curator's-Pet as effectively a free pass (5W/0L in playtest; sim 94–95% under generous mods). Approach: pair each generous mod with a downside the player can play around (interest-cap shrink, per-bench tax, Refit-cost premium, etc.) — keep them feeling generous, add bite. Acceptance: sim sweep with each generous mod lands in 30–60% band (greedy n=200, no other modifier); punishing modifiers untouched. Files: `src/modifiers.js` defs + onRound/onShop hooks; `web/app.js` description rendering if new copy needed. Buckets after B: (C) Scoring transparency — judge-reveal Continue, held-N-rounds, per-augment/item attribution; (D) Tooltip + copy cleanup; (E) Plinth L6+ + stale tier-odds; (F) Sporal Taxonomy Badge + mid-run unlock timing. Held / deferred: Crystalline/Plasmic species buffs (need targeted playtest), 0.68 judge-scale removal, Puffzak T2 aura, 66 unit tests rewrite, `src/ranking.js` cleanup, Refit-pool weighting follow-ups.
